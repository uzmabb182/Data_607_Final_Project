...
```{r}
census_df$NAME<-gsub(" County, New Jersey","",as.character(census_df$NAME))
...
```{r}
census_df <- select(census_df, county, population, median_age, household_income, per_capita_income, poverty_count, unemployment_count)
...
new_df <- merge_df %>% inner_join(education_df, by="fips_code")
...
```{r}
model <- lm(unemployment_count ~ household_income + per_capita_income + poverty_count, data = new_df)
summary(model)
```
...
```{r}
sigma(model)/mean(new_df$unemployment_count)
```
